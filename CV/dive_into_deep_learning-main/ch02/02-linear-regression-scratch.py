...
def create_data(w, b, nums_example):
    X = torch.normal(0, 1, (nums_example, len(w)))  #随机生成特征
...
def loss(y_hat, y):
    ...
    return (y_hat - y.reshape(y_hat.shape)) ** 2 / 2  #列向量变为行向量
...
# 训练模型
lr = 0.03           #学习率
num_epochs = 3      #轮次：把整个训练集完整地送入模型中一次。
...
for epoch in range(0, num_epochs):
    ...
    with torch.no_grad():   #只打算要结果，不打算反向传播
